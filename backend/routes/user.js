...
router.get('/:username', async (req, res) => {
  try {
    ...
    if (error || !user) return res.status(404).json({ hata: true, mesaj: 'Kullanıcı bulunamadı.' });
    ...
    res.status(500).json({ hata: true, mesaj: 'Sunucu hatası.' });
  }
});
...
router.put('/profile', async (req, res) => {
  try {
    ...
    if (error) return res.status(400).json({ hata: true, mesaj: 'Profil güncellenemedi.' });
...
router.post('/follow/:userId', async (req, res) => {
  try {
    ...
    if (existing) {
      ...
      return res.json({ hata: false, durum: 'unfollowed' });
    } else {
      await supabase.from('follows').insert([{ follower_id: user.id, following_id: followingId, created_at: new Date().toISOString() }]);
      return res.json({ hata: false, durum: 'followed' });
    }
  } catch (error) {
    res.status(500).json({ hata: true, mesaj: 'Sunucu hatası.' });
  }
...
router.post('/favori-anime', async (req, res) => {
  try {
    ...
    if (mevcut) {
      await supabase.from('favorite_animes').delete().eq('id', mevcut.id);
      return res.json({ hata: false, durum: 'kaldirildi', mesaj: 'Favori animelerden kaldırıldı.' });
    }
    ...
    if (count >= 5) {
      return res.status(400).json({ hata: true, mesaj: 'En fazla 5 favori anime ekleyebilirsiniz.' });
    }
...
router.get('/anime-ara', async (req, res) => {
  try {
    ...
    const url = 'https://api.jikan.moe/v4/anime?q=' + encodeURIComponent(arama) + '&limit=8&order_by=popularity';
    ...
    https.get(url, (response) => {
      ...
      response.on('end', () => {
        try {
          ...
          const animeler = (parsed.data || []).map(a => ({
            anime_id: a.mal_id,
            anime_name: a.title,
            anime_image: a.images?.jpg?.image_url || a.images?.jpg?.small_image_url || ''
          }));
